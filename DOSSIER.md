# Athlete Dossier

## Identity
- **Sport(s):** Running
- **Experience:** < 1 year consistent training

## Physiology
- **Primary metrics:** Heart rate and pace (no power meter)
- **Resting heart rate:** 57 bpm
- **Max heart rate:** 204 bpm
- **Lactate threshold HR (LTHR):** 181 bpm
- **FTP:** N/A (running athlete, pace/HR based)

## Training Structure
- **Weekly runs:** 5
  - Tuesday: Hard session (threshold)
  - Thursday: Hard session (intervals)
  - Saturday: Long run
  - Remaining days: Easy runs
  - Monday: Rest day (off-limits)
- **Weekly hours:** Flexible, no fixed hour target

## Goals
- **Primary goal:** Sub 1:30 half marathon
- **Target races:**
  - 10K — 31 May 2026
  - Half Marathon — 25 October 2026

## Health & Limitations
- No current injuries
- No past injuries affecting training
- No health conditions

## Preferences
- **Plan style:** Structured
- **Environment:** Outdoor (preferred)
- **Hard session days:** Tuesday, Thursday
- **Long run day:** Saturday
- **Rest day:** Monday

## Additional Training
- **Mobility:** Yes — include mobility sessions in weekly plan
- **Bodyweight strength:** Yes — include bodyweight strength sessions in weekly plan
- **Preferred days for strength/mobility:** e.g. Wednesday and/or post-easy runs
- **Session length preference:** e.g. 20–30 minutes
- **Equipment available:** Bodyweight only, no gym
